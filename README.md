...
Chatbot currently at **v1.4.4**, Evaluator at **v1.3.11** — see [Changelog](#-changelog).
...
**v1.4.4**
- Quick actions now tag the artifact type they produce (e.g. "Generate a full IEEE 829 test plan" → Test Plan), passed to the Evaluator via sessionStorage and embedded as metadata in TXT/MD/HTML exports — replaces guessing the type from output text after the fact with a deterministic signal from generation time

**v1.4.3**
- Added "Clear all" button to the attachment preview row (shown once there are 2+ pending attachments)
...
**v1.3.11**
- Reads artifact-type metadata (from chatbot, via sessionStorage or embedded file markers) before falling back to keyword-based auto-detect — a manual dropdown selection always overrides both

**v1.3.10**
- Added automatic HTML-export detection and stripping — pasting the contents of an exported `.html` file (instead of the plain markdown/text response) now correctly extracts the visible content before auto-detect and scoring, instead of choking on the raw CSS/markup

**v1.3.9**
- Follow-up fix for auto-detect misclassification (v1.3.8's frequency-weighting helped but wasn't sufficient on its own) — now also heavily weights keywords found in the document's title/first heading, since an explicit self-declared title is a far more reliable signal than word frequency in body prose

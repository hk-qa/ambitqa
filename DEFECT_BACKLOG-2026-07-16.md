...
| DEF-013 | Auto-detect misclassifies Test Plan as Bug Report | High | evaluator.html | Fixed | evaluator v1.3.9 (v1.3.8 fix was incomplete) |
| DEF-014 | Evaluation fails with "Unterminated string in JSON" error | Medium | evaluator.html | Fixed | evaluator v1.3.8 |
| DEF-015 | Pasting an HTML export breaks auto-detect and pollutes evaluation | High | evaluator.html | Fixed | evaluator v1.3.10 |
...
**Severity:** High · **Component:** evaluator.html · **Fixed in:** evaluator v1.3.9 (v1.3.8 fix was incomplete)
...
Bug Report keyword) counted equally toward that other rubric.

**First resolution attempt (v1.3.8):** Changed scoring from boolean
presence to counting total keyword occurrences. Fixed the originally
reported case, but the underlying defect recurred on a different
document — a short test plan whose body happened to reuse generic QA
vocabulary ("environment," "priority," "severity") heavily enough that
Bug Report still numerically won on frequency alone, even against a
document titled "HustlHub Website Test Plan" in its very first heading.

**Follow-up fix (v1.3.9):** Added a heavy scoring bonus (+10) for
keywords found specifically in the document's title/first markdown
heading, since an explicit self-declared title ("## X Test Plan") is a
far more reliable signal than word frequency in body prose. Caught and
fixed a regression during this change before shipping: the initial
implementation used a "first 150 characters" fallback for documents with
no markdown heading, which on short documents effectively covered nearly
the entire body — handing the title bonus to multiple unrelated keywords
indiscriminately rather than reflecting a genuine authorial signal, and
broke a previously-passing case. Restricted the bonus to only apply when
a real heading exists; short/plain-prose documents fall back to pure
frequency counting (the v1.3.8 behavior, which was already correct for
that case). Verified via 5 test cases covering the original failure, the
near-regression, and three other document types — all correct with no
regressions.
...
### DEF-015 — Pasting an HTML export breaks auto-detect and pollutes evaluation
**Severity:** High · **Component:** evaluator.html · **Fixed in:** evaluator v1.3.10

**Description:** Pasting the contents of an exported `.html` file (e.g.
from chatbot's HTML export button) instead of the plain markdown/text
response caused auto-detect to misclassify the artifact (Test Plan →
Bug Report) and produced a nonsensical FAIL 0/100 verdict.

**Root cause:** Two compounding issues. First, the title-detection logic
added for DEF-013 specifically looks for markdown headings (`## Text`)
— an HTML export uses `<h1>`/`<h2>` tags instead, so no title signal was
found at all. Second, and more fundamentally, the raw HTML/CSS source
(hundreds of characters of `font-family`, `padding`, `border`, etc.) got
sent as-is to both `detectType()` and the scoring LLM itself, diluting
the actual content signal and giving the model a much harder, noisier
document to reason about than intended.

**Resolution:** Added `extractPlainTextIfHtml()` — detects whether the
pasted content is a full HTML document (`<!DOCTYPE html>` or `<html>` in
the first ~500 characters) and, if so, extracts just the visible text
using the browser's own HTML parser (via a detached DOM element, with
`<script>`/`<style>` elements explicitly removed before reading
`textContent`) rather than a hand-rolled regex stripper. Runs at the
start of `runEvaluation()`, before type detection or the LLM call, and
updates the visible textarea to show the cleaned text so what's
displayed matches what's actually evaluated. Verified end-to-end: a
reconstructed HTML export (CSS + a test plan buried in `<h1>`/`<h2>`
tags) now correctly extracts to clean text, classifies as Test Plan, and
the actual LLM request payload contains zero CSS/markup.

...
### Done
- [x] Tag artifact type at generation time instead of guessing it after
  the fact in the Evaluator — root cause of DEF-002/013/015 was
  fundamentally that the Evaluator had to guess artifact type from
  output text alone. Full design was captured in this file previously;
  implemented as: `modeInfo.actions` restructured from plain strings to
  `{text, type}` objects (70 quick actions across all 11 modes, each
  tagged with the Evaluator rubric key it produces — 63 tagged, 7
  intentionally left untagged where no rubric cleanly fits, e.g.
  exploratory charters); a transient `quickActionTypeHint` captured once
  per response and cleared immediately after, so it can't leak onto a
  later free-typed message; passed to the Evaluator two ways — via
  `sessionStorage` (`ambitqa_eval_type`) for the Evaluate-button flow,
  and embedded as real metadata in downloaded files (HTML `<meta>` tag,
  MD comment, TXT header line) so a file opened in a later session still
  carries its type. Verified end-to-end: all 70 mappings cross-checked
  against real rubric keys (zero typos), and a document deliberately
  engineered to reproduce the exact known misclassification pattern
  correctly resolves via metadata instead of the flawed keyword path.
- [x] Added "🗑 Clear all" button to the attachment preview row — shown
  only when there are 2+ pending attachments (a single one is already
  covered by its own × remove button), clearing all pending attachments
  in one click
...
### Done
- [x] Reads artifact-type metadata (via `detectEmbeddedType()`) before
  falling back to keyword-based `detectType()` — checks sessionStorage
  (`ambitqa_eval_type`, set by chatbot before the Evaluate button opens
  this tab) and, for pasted/uploaded content, an embedded marker in the
  text itself (HTML `<meta>` tag, MD comment, or TXT header line). A
  manual type selection in the dropdown always overrides both. Full
  cross-file design/implementation notes under chatbot.html's matching
  entry above.
